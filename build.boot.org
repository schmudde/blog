...
#+AUTHOR: David Schmudde (clojure and literate/org-babel)
#+EMAIL: d@schmud.de
#+LANGUAGE: en
#+PROPERTY: tangle build.boot

This README is also the ~build.boot~ file as a literate document. ~CTRL-c-v-t~ [[https://orgmode.org/org.html#Working-With-Source-Code][will tangle code]]; Org will process each code block and generate a source file called ~build.boot~.
